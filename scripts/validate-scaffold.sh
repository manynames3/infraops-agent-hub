...
json_files = sorted(path for path in Path(".").glob("**/*.json") if ".git" not in path.parts)
...
if grep -R -E "(AKIA[0-9A-Z]{16}|xox[baprs]-|ghp_[A-Za-z0-9_]{36,}|sk-[A-Za-z0-9]{20,})" . --exclude-dir=.git >/tmp/infraops-secret-scan.txt; then
